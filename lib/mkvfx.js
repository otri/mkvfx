...
var mkvfx_source_root = cwd + "/extern"
var mkvfx_build_root = cwd + "/build"
	...
	for (var i = 0; i < package_recipes.length; ++i) {
		if (package_recipes[i].name === package_name) {
			...
			if ("recipe_osx" in p) {
				...
				cursor.red()
				console.log("No recipe_osx exists for " + package_name)
				cursor.fg.reset()
				throw new Error("No recipe_osx exists for " + package_name)
			}
